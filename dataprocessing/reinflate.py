...
def process_csvs(
    input_r_filename: str = "fits/0_Rt.csv",
    input_exceed_filename: str = "fits/0_Pexceed.csv",
    input_Bweekly_filename: str = "fits/0_Bweekly.csv",
    input_Cweekly_filename: str = "fits/0_Cweekly.csv",
    input_Bpred_filename: str = "fits/0_Bpred.csv",
    input_Bproj_filename: str = "fits/0_Bproj.csv",
    ...
    output_Bweekly_filename: str = "website/default/Bweekly.csv",
    output_Cweekly_filename: str = "website/default/Cweekly.csv",
    output_Bpred_filename: str = "website/default/Bpred.csv",
    output_Bproj_filename: str = "website/default/Bproj.csv",
    ...
    reproduction_numbers = pd.read_csv(input_r_filename).rename(columns={"Date": "day"})
    reproduction_numbers["day"] = pd.to_datetime(
        reproduction_numbers["day"], format="%Y-%m-%d"
    )
    reproduction_numbers = reproduction_numbers.set_index(["area", "day"])

    exceedance_probs = pd.read_csv(input_exceed_filename).rename(
        columns={"Date": "day"}
    )
    exceedance_probs["day"] = pd.to_datetime(exceedance_probs["day"], format="%Y-%m-%d")
    exceedance_probs = exceedance_probs.set_index(["area", "day"])

    Bweekly = pd.read_csv(input_Bweekly_filename).rename(columns={"Date": "day"})
    Bweekly["day"] = pd.to_datetime(Bweekly["day"], format="%Y-%m-%d")
    Bweekly = Bweekly.set_index(["area", "day"])

    Cweekly = pd.read_csv(input_Cweekly_filename).rename(columns={"Date": "day"})
    Cweekly["day"] = pd.to_datetime(Cweekly["day"], format="%Y-%m-%d")
    Cweekly = Cweekly.set_index(["area", "day"])

    Bpredictions = pd.read_csv(input_Bpred_filename).rename(columns={"Date": "day"})
    Bpredictions["day"] = pd.to_datetime(Bpredictions["day"], format="%Y-%m-%d")
    Bpredictions = Bpredictions.set_index(["area", "day"])

    Bprojections = pd.read_csv(input_Bproj_filename).rename(columns={"Date": "day"})
    Bprojections["day"] = pd.to_datetime(Bprojections["day"], format="%Y-%m-%d")
    Bprojections = Bprojections.set_index(["area", "day"])

    Cpredictions = pd.read_csv(input_Cpred_filename).rename(columns={"Date": "day"})
    Cpredictions["day"] = pd.to_datetime(Cpredictions["day"], format="%Y-%m-%d")
    Cpredictions = Cpredictions.set_index(["area", "day"])

    Cprojections = pd.read_csv(input_Cproj_filename).rename(columns={"Date": "day"})
    Cprojections["day"] = pd.to_datetime(Cprojections["day"], format="%Y-%m-%d")
    Cprojections = Cprojections.set_index(["area", "day"])

    Xpredictions = pd.read_csv(input_Xpred_filename).rename(columns={"Date": "day"})
    Xpredictions["day"] = pd.to_datetime(Xpredictions["day"], format="%Y-%m-%d")
    Xpredictions = Xpredictions.set_index(["area", "day"])

    Xprojections = pd.read_csv(input_Xproj_filename).rename(columns={"Date": "day"})
    ...
    Bweekly.index.names = ["area", "Date"]
    Cweekly.index.names = ["area", "Date"]
    Bpredictions.index.names = ["area", "Date"]
    Bprojections.index.names = ["area", "Date"]
    ...
    Bweekly.to_csv(output_Bweekly_filename, float_format="%3d")
    Cweekly.to_csv(output_Cweekly_filename, float_format="%3d")
    Bpredictions.to_csv(output_Bpred_filename, float_format="%2.1f")
    Bprojections.to_csv(output_Bproj_filename, float_format="%2.1f")
...
def reinflate(
    reproduction_numbers: pd.DataFrame,
    exceedance_probs: pd.DataFrame,
    Bweekly: pd.DataFrame,
    Cweekly: pd.DataFrame,
    Bpredictions: pd.DataFrame,
    Bprojections: pd.DataFrame,
    ...
    england_map = england_map.merge(
        metadata["POPULATION"], left_on=["Meta area"], right_on=["AREA"], how="left"
    ).merge(metadata["POPULATION"], left_on=["area"], right_on=["AREA"], how="left")
    england_map["ratio"] = england_map["POPULATION_y"] / england_map["POPULATION_x"]
    ...
    scotland_map = scotland_map.merge(
        metadata[["POPULATION", "CODE"]],
        left_on=["NHS Scotland Health Board"],
        right_on=["AREA"],
        how="left",
    )
    scotland_map = scotland_map[scotland_map.CODE == "NHS Scotland Health Board"].drop(
        columns=["CODE"]
    )

    scotland_map = scotland_map.merge(
        metadata[metadata.CODE != "NHS Scotland Health Board"]["POPULATION"],
        left_on=["area"],
        right_on=["AREA"],
        how="left",
    )

    scotland_map["ratio"] = scotland_map["POPULATION_y"] / scotland_map["POPULATION_x"]
    ...
    england = (
        england_map.merge(
            reproduction_numbers.reset_index(level=1),
            left_on=["Meta area"],
            right_on=["area"],
            how="left",
        )
    ...
    scotland = (
        ...
        .set_index(["area", "day"])
        .drop(columns=["NHS Scotland Health Board", "ratio"])
    )

    reproduction_numbers = reproduction_numbers.append(england).append(scotland)
    ...
    england = (
        england_map.merge(
            exceedance_probs.reset_index(level=1),
            left_on=["Meta area"],
            right_on=["area"],
            how="left",
        )
    ...
    exceedance_probs = exceedance_probs.append(england).append(scotland)

    # Bweekly Cases
    england = england_map.merge(
        Bweekly.reset_index(level=1),
        left_on=["Meta area"],
        right_on=["area"],
        how="left",
    )
    ...
    if divide_predictions_by_population_ratio:
        england["B_weekly"] = england["B_weekly"] * england["ratio"]

    england = england.drop(columns=["Meta area", "ratio"]).set_index(["area", "day"])

    scotland = scotland_map.merge(
        Bweekly.reset_index(level=1),
        left_on=["NHS Scotland Health Board"],
        right_on=["area"],
        how="left",
    ...
    if divide_predictions_by_population_ratio:
        scotland["B_weekly"] = scotland["B_weekly"] * scotland["ratio"]

    scotland = scotland.drop(columns=["NHS Scotland Health Board", "ratio"]).set_index(
        ["area", "day"]
    )
    ...
    # Case predictions without Cweekly variation
    england = england_map.merge(
        Bpredictions.reset_index(level=1),
        left_on=["Meta area"],
        right_on=["area"],
        how="left",
    )
    ...
    # Case predictions
    england = england_map.merge(
        Cpredictions.reset_index(level=1),
        left_on=["Meta area"],
        right_on=["area"],
        how="left",
    )
    ...
    # Case projections without weekly variation
    england = england_map.merge(
        Bprojections.reset_index(level=1),
        left_on=["Meta area"],
        right_on=["area"],
        how="left",
    )
    ...
    # Case projections
    england = england_map.merge(
        Cprojections.reset_index(level=1),
        left_on=["Meta area"],
        right_on=["area"],
        how="left",
    )
    ...
    england = england_map.merge(
        Xpredictions.reset_index(level=1),
        left_on=["Meta area"],
        right_on=["area"],
        how="left",
    )
    ...
    england = england_map.merge(
        Xprojections.reset_index(level=1),
        left_on=["Meta area"],
        right_on=["area"],
        how="left",
    )
    ...
    Bweekly = Cweekly.loc[~Bweekly.index.duplicated(keep="first")]
    Cweekly = Cweekly.loc[~Cweekly.index.duplicated(keep="first")]
    Bpredictions = Bpredictions.loc[~Bpredictions.index.duplicated(keep="first")]
    Bprojections = Bprojections.loc[~Bprojections.index.duplicated(keep="first")]
    Cpredictions = Cpredictions.loc[~Cpredictions.index.duplicated(keep="first")]
    Cprojections = Cprojections.loc[~Cprojections.index.duplicated(keep="first")]
    Xpredictions = Xpredictions.loc[~Xpredictions.index.duplicated(keep="first")]
    Xprojections = Xprojections.loc[~Xprojections.index.duplicated(keep="first")]

    return (
        reproduction_numbers.sort_index().dropna(),
        exceedance_probs.sort_index().dropna(),
        Bweekly.sort_index().dropna(),
        Cweekly.sort_index().dropna(),
        Bpredictions.sort_index().dropna(),
        Bprojections.sort_index().dropna(),
...
if __name__ == "__main__":
    ...
    if len(args) == 20:
        process_csvs(
            input_r_filename=args[0],
            input_exceed_filename=args[1],
            input_Bweekly_filename=args[2],
            input_Cweekly_filename=args[3],
            input_Bpred_filename=args[4],
            input_Bproj_filename=args[5],
            input_Cpred_filename=args[6],
            input_Cproj_filename=args[7],
            input_Xpred_filename=args[8],
            input_Xproj_filename=args[9],
            output_r_filename=args[10],
            output_exceed_filename=args[11],
            output_Bweekly_filename=args[12],
            output_Cweekly_filename=args[13],
            output_Bpred_filename=args[14],
            output_Bproj_filename=args[15],
            output_Cpred_filename=args[16],
            output_Cproj_filename=args[17],
            output_Xpred_filename=args[18],
            output_Xproj_filename=args[19],
        )
    elif len(args) == 10:
        process_csvs(
            input_r_filename=args[0],
        ...
        raise Error("Number of arguments has to be 9 or 18")
