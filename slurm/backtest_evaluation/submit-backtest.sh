...
function clean_and_map { # options_clean = $1, first_day_modelled = $2, results_directory = $3, options_map = $4
    sbatch --wait \
    ...
    sbatch --wait \
        --mail-user=$USER@stats.ox.ac.uk \
        --mail-type=ALL \
        --job-name=Rmap_regional_start_"$2" \
        --output="$3"/regional/output/run_%A_%a.out \
        --partition=ziz-large \
        --ntasks=1 \
        --cpus-per-task=1 \
        --mem-per-cpu=10G \
...
do
    ...
    do
        ...
        clean_and_map "$options_clean" "$weeks_modelled" "$results_directory" "$options_map" &
    ...
    echo "Completed a model config backtest."
done
